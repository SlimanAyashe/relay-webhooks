...
def _validate_url(url: str) -> None:
    ...
    if parsed.scheme != "https":
        raise ValueError(f"endpoint url must be https: {url!r}")
    if not parsed.netloc:
        raise ValueError(f"endpoint url must have a host: {url!r}")


def _validate_event_types(event_types: frozenset[str]) -> None:
    if not event_types:
        raise ValueError("subscribed_event_types must not be empty")
    if any(not event_type.strip() for event_type in event_types):
        raise ValueError("subscribed_event_types must not contain blank entries")
